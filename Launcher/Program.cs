...
internal sealed partial class MainForm : Form
{
    ...
    private async Task RunSelectedToolAsync()
    {
        ...
        try
        {
            var psi = new ProcessStartInfo
            {
                ...
                Arguments = BuildPowerShellCaptureArguments(scriptPath, _argsTextBox.Text),
    ...
    private static string BuildPowerShellCaptureArguments(string scriptPath, string rawArgs)
    {
        var invocation = string.IsNullOrWhiteSpace(rawArgs)
            ? $"& {Quote(scriptPath)}"
            : $"& {Quote(scriptPath)} {rawArgs}";

        var command = $"{invocation} *>&1 | Out-String -Width 500";

        return $"-NoProfile -ExecutionPolicy Bypass -Command {Quote(command)}";
    }
